 ...
 * Supports both File System Access API (Chrome/Edge), fallback (Firefox/Safari), and Android WebView
 */
...
let canSaveToFolder = false;  // True only with File System Access API or Android
let gameInfo = {
    game: '',
    folderName: '',
    linkIdentifier: '',
...
// Folder selection - uses Android bridge, File System Access API, or fallback
async function selectFolder() {
    // Android WebView - use native folder picker
    if (isAndroidApp) {
        return new Promise((resolve) => {
            // Define callback function that Android will call
            window.onAndroidFolderSelected = (name) => {
                if (name) {
                    folderName = name;
            ...
            AndroidBridge.selectFolder('window.onAndroidFolderSelected');
        });
    }
...
async function saveGame() {
    ...
    const jsonContent = JSON.stringify(gameInfo, null, 2);
    
    // Android WebView - use native save
    if (isAndroidApp && canSaveToFolder) {
        AndroidBridge.saveGameInfo(jsonContent, folderName);
        // Android will show toast and call window.onAndroidSaveComplete
        return;
    }
